...
// 1. 가격이 60000 이하이고, 멀티ㅡㄹ레이가 되는 게임 리스트
const quiz1 = nintendo.filter((x)=> x.price.every((v)=> v < 60000) && x.multiplayer)
console.log (quiz1)

// 2. 장르가 어드벤처이고ㅓ, 가격이 60000 원 이상인 게임리스트
const quiz2 = nintendo.filter ((x)=>x.genre.some((v)=> v=="어드벤처")&& x.price > 60000)
console.log (quiz2)
//3. 멀티 플레이어가 안되고ㅡ 장르가 어드벤처인 게임리스트
const quiz3 = nintendo.filter ((x)=> x.genre.some((v)=> v=="어드벤처") && multiplayer.false)
